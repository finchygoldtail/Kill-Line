...
impl Harness {
    ...
    fn obs(&mut self, kind: ObsKind, outcome: Option<Outcome>) -> Vec<Event> {
        ...
        let o = Observation {
            timestamp: self.t,
            process: ProcessInfo { pid: 100, tid: 100, ppid: 1, uid: 65534, gid: 65534, comm: "python3".into(), exe: None },
            kind,
            runtime_setup: false,
            outcome,
        };
    ...
    fn open(&mut self, path: &str, access: FileAccess) -> Event {
        self.obs(
            ObsKind::Open { path: path.into(), access, flags: 0, resolution: "lexical".into(), via: None },
...
fn net(addr: &str, port: u16) -> ObsKind {
    ObsKind::Net { op: NetOp::Connect, addr: addr.parse().unwrap(), port }
}

#[test]
fn normal_python_work_is_green() {
    let mut h = Harness::new(POLICY);
    h.obs(ObsKind::Exec { path: "/usr/local/bin/python3".into(), argv: vec!["python3".into()], sha256: None, exists: true }, Some(Outcome::Succeeded));
    for p in ["/etc/ld.so.cache", "/usr/lib/x86_64-linux-gnu/libc.so.6", "/usr/local/lib/python3.12/os.py", "/proc/self/maps", "/dev/urandom"] {
        assert_eq!(h.open(p, FileAccess::Read).verdict, Verdict::Allowed, "{}", p);
    }
    assert!(h.open("/workspace/task.md", FileAccess::Read).allowed);
    assert!(h.open("/workspace/output/report.txt", FileAccess::Write).allowed);
...
fn demo1_outbound_network_is_red_with_plain_english() {
    let mut h = Harness::new(POLICY);
    let e = h.obs(net("203.0.113.42", 443), Some(Outcome::Failed { errno: 101, error: errno_name(101) })).remove(0);
    ...
    assert!(e.explanation.contains("attempted to connect to 203.0.113.42:443"));
...
fn symlinks_and_dotdot_do_not_evade_path_rules() {
    ...
    let e = h
        ...
        .remove(0);
    assert_eq!(e.verdict, Verdict::Violation);
    assert!(e.explanation.contains("Requested as /workspace/output/innocent.txt"));
    // Lexical normalisation is done by the sensor; the engine sees clean paths.
    assert_eq!(killline_core::pathmatch::normalize("/workspace/../root/.ssh/id_rsa"), "/root/.ssh/id_rsa");
}

#[test]
fn metadata_endpoints_including_ipv4_mapped_ipv6() {
    let mut h = Harness::new(POLICY);
    for a in ["169.254.169.254", "::ffff:169.254.169.254", "fd00:ec2::254", "168.63.129.16"] {
        let e = h.obs(net(a, 80), None).remove(0);
        assert_eq!(e.category, Category::CloudMetadata, "{}", a);
        assert_eq!(e.severity, Severity::Critical);
    }
    let e = h.obs(ObsKind::Dns { server: None, query: Some("metadata.google.internal".into()) }, None).remove(0);
...
fn docker_socket_is_critical_via_connect_and_open() {
    let mut h = Harness::new(POLICY);
    let e = h.obs(ObsKind::UnixConnect { path: "/var/run/docker.sock".into(), abstract_ns: false }, None).remove(0);
    ...
    let e = h.obs(ObsKind::UnixConnect { path: "/var/run/nscd/socket".into(), abstract_ns: false }, None).remove(0);
...
fn escape_indicators() {
    let mut h = Harness::new(POLICY);
    assert_eq!(h.open("/proc/sys/kernel/core_pattern", FileAccess::Write).category, Category::ContainerEscape);
    assert_eq!(h.open("/sys/fs/cgroup/memory/x/release_agent", FileAccess::Write).category, Category::ContainerEscape);
    assert_eq!(h.open("/proc/1/root/etc/passwd", FileAccess::Read).category, Category::ContainerEscape);
    // Own /proc entries are fine.
    assert_ne!(h.open("/proc/100/environ", FileAccess::Read).category, Category::Credential);
    assert_eq!(h.open("/proc/555/environ", FileAccess::Read).category, Category::Credential);
    let e = h.obs(ObsKind::Setns { nstype: 0x2000_0000 }, None).remove(0);
    assert_eq!(e.category, Category::Namespace);
    let e = h.obs(ObsKind::Mount { source: "/dev/sda1".into(), target: "/mnt".into(), flags: 0 }, None).remove(0);
...
fn privilege_and_tamper() {
    let mut h = Harness::new(POLICY);
    let e = h.obs(ObsKind::SetId { call: "setuid".into(), args: vec![0] }, None).remove(0);
    assert_eq!(e.action, "privilege.become_root");
    let e = h.obs(ObsKind::Kill { target_pid: 4242, signal: 9 }, None).remove(0);
    assert_eq!(e.category, Category::Tamper);
    assert_eq!(e.verdict, Verdict::Violation);
    let e = h.obs(ObsKind::Chmod { path: "/workspace/output/x".into(), mode: 0o4755 }, None).remove(0);
...
fn process_policy() {
    let mut h = Harness::new(POLICY);
    let ex = |p: &str, exists| ObsKind::Exec { path: p.into(), argv: vec![], sha256: None, exists };
    assert_eq!(h.obs(ex("/usr/bin/nc", true), None)[0].action, "process.exec_denied");
    assert_eq!(h.obs(ex("/usr/bin/sudo", true), None)[0].action, "process.exec_denied");
    assert_eq!(h.obs(ex("/usr/bin/sh", true), None)[0].action, "process.exec_unexpected");
    assert_eq!(h.obs(ex("/usr/bin/su", true), None)[0].action, "process.exec_privileged");
    // PATH-search probes that fail with ENOENT are not violations...
    let e = h.obs(ex("/root/.local/bin/sh", true), Some(Outcome::Failed { errno: 2, error: errno_name(2) })).remove(0);
...
fn runtime_setup_is_recorded_not_evaluated() {
    ...
    let o = Observation {
        timestamp: Utc::now(),
        process: ProcessInfo { pid: 7, comm: "runc:[2:INIT]".into(), ..Default::default() },
        kind: ObsKind::Open { path: "/self/setgroups".into(), access: FileAccess::Path, flags: 0, resolution: "lexical".into(), via: None },
        runtime_setup: true,
        outcome: None,
    };
...
fn benign_failed_bytecode_writes_are_not_violations_but_successful_ones_are() {
    ...
    let e = h
        .obs(
            ObsKind::Open { path: p.into(), access: FileAccess::Write, flags: 0o101, resolution: "lexical".into(), via: None },
            Some(Outcome::Failed { errno: 30, error: errno_name(30) }),
        )
        .remove(0);
...
fn demo3_behaviour_shift_goes_amber_then_red_with_possible_correlation() {
    ...
    for i in 0..120 {
        let evs = h.obs(
            ObsKind::Open { path: format!("/workspace/corpus/doc-{}.txt", i), access: FileAccess::Read, flags: 0, resolution: "lexical".into(), via: None },
            Some(Outcome::Succeeded),
        );
        if evs.iter().any(|e| e.verdict == Verdict::Anomaly) {
            ...
            assert!(a.correlations.iter().any(|c| c.summary.starts_with("Possible correlation")));
        }
    }
    assert!(saw_amber);
    let cred = h.open("/fake-secrets/api-key.txt", FileAccess::Read);
    assert_eq!(h.session.status, Status::Red);
    assert!(cred.correlations.iter().any(|c| c.summary.contains("untrusted input /workspace/untrusted/README.md")));
    let n = h.obs(net("203.0.113.42", 443), None).remove(0);
    let text: Vec<&str> = n.correlations.iter().map(|c| c.summary.as_str()).collect();
    assert!(text.iter().any(|t| t.starts_with("Possible exfiltration pattern")), "{:?}", text);
    assert!(text.iter().any(|t| t.starts_with("Follows a behavioural anomaly")));
}
...
"#;
    ...
    assert_eq!(e.verdict, Verdict::Violation, "direct IP without prior allowed DNS");
    assert!(h.obs(ObsKind::Dns { server: None, query: Some("raw.githubusercontent.com".into()) }, None)[0].allowed);
    let e = h.obs(net("185.199.108.133", 443), None).remove(0);
    assert!(e.allowed);
    assert!(e.explanation.contains("not verified"));
    let e = h.obs(ObsKind::Dns { server: None, query: Some("evil.example".into()) }, None).remove(0);
    assert_eq!(e.verdict, Verdict::Violation);
}
...
fn never_claims_safety() {
    ...
    for e in &h.events {
        ...
        assert!(!t.contains("is safe") && !t.contains("are safe") && !t.contains("secure"), "{}", e.explanation);
    }
