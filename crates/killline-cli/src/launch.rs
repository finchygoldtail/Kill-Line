...
//! On Windows the agent is created suspended instead, added to the tracked
//! set, then resumed.
...
mod windows {
    ...
    impl Launched {
        pub fn release(&mut self) -> Result<()> {
            ...
            #[link(name = "ntdll")]
            extern "system" {
                fn NtResumeProcess(h: *mut core::ffi::c_void) -> i32;
            }
            // SAFETY: the handle belongs to our live child.
            let status = unsafe { NtResumeProcess(self.child.as_raw_handle()) };
            if status < 0 {
                bail!("could not resume the agent (NTSTATUS {:#x})", status);
            }
            self.resumed = true;
            Ok(())
        }
    ...
    pub fn spawn(command: &[String], uid: Option<u32>, gid: Option<u32>) -> Result<Launched> {
        if uid.is_some() || gid.is_some() {
            bail!("--user is not supported on Windows; run the agent under a standard user account instead");
        }
