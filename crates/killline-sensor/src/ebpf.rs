...
const TRACEPOINTS: &[(&str, &str, &str, bool, &str)] = &[
    (
        ...
        "sched_process_fork",
        true,
        "child processes would not be tracked",
    ),
    (
        "kl_exit",
        "sched",
...
impl Sensor {
    ...
    /// Whether this build can monitor anything on this platform.
    ...
    pub fn start(scope: &Scope) -> Result<Sensor> {
        ...
        {
            let mut cfg: Array<_, KlConfig> = Array::try_from(
                bpf.map_mut("config")
            ...
            cfg.set(
                0,
                KlConfig {
                    target_pidns: scope.pidns.unwrap_or(0),
                    monitor_tgid: std::process::id(),
                },
                0,
            )?;
        }
        {
            let mut tracked: BpfHashMap<_, u32, u8> = BpfHashMap::try_from(
                bpf.map_mut("tracked")
                    .ok_or_else(|| anyhow!("tracked map missing"))?,
            )?;
        ...
        for (prog, cat, name, critical, what) in TRACEPOINTS {
            let res = (|| -> Result<()> {
                let tp: &mut TracePoint = bpf
                    .program_mut(prog)
                    .ok_or_else(|| anyhow!("program {} missing", prog))?
                    .try_into()?;
                tp.load()?;
                tp.attach(cat, name)?;
                Ok(())
            ...
            coverage.push(CoverageItem {
                ...
                detail: res
                    .err()
                    .map(|e| format!("unavailable ({}): {} not observed", root_cause(&e), what)),
            });
        }
        ...
        let core_results_ok = !failed_results.iter().any(|f| {
            f.starts_with("openat ") || f.starts_with("connect ") || f.starts_with("execve ")
        });
        coverage.push(CoverageItem {
        ...
        let res = (|| -> Result<()> {
            let p: &mut FEntry = bpf
                .program_mut("kl_file_open")
                .ok_or_else(|| anyhow!("kl_file_open missing"))?
                .try_into()?;
    ...
    pub fn poll(
        ...
        self.decoder
            .flush_stale(std::time::Duration::from_millis(250), out);
        Ok(n)
    }
    ...
    /// A reason the sensor can no longer see, if any. (The eBPF programs
    /// live as long as this process holds them.)
    pub fn health(&self) -> Option<String> {
        None
    }

    pub fn tracked_pids(&self) -> Result<Vec<u32>> {
        let tracked: BpfHashMap<_, u32, u8> = BpfHashMap::try_from(
            self.bpf
                .map("tracked")
                .ok_or_else(|| anyhow!("tracked map missing"))?,
        )?;
