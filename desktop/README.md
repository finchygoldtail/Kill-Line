...
A native window around the KillLine dashboard, built with [Tauri 2](https://tauri.app). Linux only for now.

## What happens when you open it

1. The app shows a start screen and asks the system for **administrator permission** (polkit / `pkexec`). KillLine needs root to load its observe-only eBPF sensor into the kernel.
...
4. You get **native desktop notifications** when an agent crosses a boundary, or when KillLine loses visibility (GREY).
...
The output is `desktop/src-tauri/target/release/bundle/deb/KillLine_0.1.0_amd64.deb` and an AppImage. Install the `.deb` with `sudo apt install ./KillLine_0.1.0_amd64.deb` and launch **KillLine** from the applications menu.
