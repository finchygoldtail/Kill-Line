...
got_pipe = any(e["category"] == "container_runtime" for e in v)
print(f"[INFO] Docker named pipe open {'seen' if got_pipe else 'not seen (known V1 gap: Kernel-File does not report named-pipe opens; docs/LIMITATIONS.md)'}")
...
check("tcp attempt that never completes (203.0.113.42)", got_testnet)
